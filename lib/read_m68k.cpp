...
//  * -(a0)
//  * (a0)+
std::pair<Inst *, unsigned> Parser::get_addr_basic_indirect(unsigned idx, uint32_t access_bitsize)
{
  bool minus = false;
  ...
  get_right_paren(idx++);
  if (!minus && is_kind(idx, Lexeme::plus))
    {
      assert(!minus);
      Inst *offset = bb->value_inst(access_bitsize / 8, 32);
...
std::pair<Inst *, unsigned> Parser::get_addr_other(unsigned idx)
{
  ...
  Inst *value = nullptr;
  if (!minus && is_kind(idx, Lexeme::name))
    {
  ...
  else if (minus || is_kind(idx, Lexeme::hex) || is_kind(idx, Lexeme::integer))
    {
...
void Parser::process_call()
{
  ...
  if (name == "__ctzdi2")
    {
      ...
      Inst *res = bb->build_trunc(gen_ctz(bb, arg), 32);
      bb->build_inst(Op::WRITE, rstate->registers[M68kRegIdx::d0], res);
      return;
    }
  if (name == "__divdi3")
  ...
  if (name == "exit")
    {
      Inst *sp = bb->build_inst(Op::READ, rstate->registers[M68kRegIdx::a7]);
      Inst *exit_val = bb->build_inst(Op::LOAD_BE, sp, 4);

      Inst *b1 = bb->value_inst(1, 1);
      exit_val = bb->build_trunc(exit_val, 32);
      bb->build_inst(Op::WRITE, rstate->registers[M68kRegIdx::exit], b1);
      bb->build_inst(Op::WRITE, rstate->registers[M68kRegIdx::exit_val],
		     exit_val);
      bb->build_br_inst(rstate->exit_bb);
      bb = func->build_bb();
      return;
