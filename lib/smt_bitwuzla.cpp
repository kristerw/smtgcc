...
std::pair<SStats, Solver_result> check_refine_bitwuzla(Function *func)
{
  ...
  if (need_checking_retval(conv.src, conv.tgt))
	...
	{
	  Term src_indef = conv.inst_as_bv(conv.src.retval_indef);
	  ...
	  if (conv.tgt.retval_indef != conv.src.retval_indef)
	    {
	      Term tgt_indef = conv.inst_as_bv(conv.tgt.retval_indef);
	      Term indef_result =
		tm.mk_term(Kind::BV_AND, {tgt_indef, src_mask});
	      Term zero =
		tm.mk_bv_zero(tm.mk_bv_sort(conv.tgt.retval->bitsize));
  ...
  if (need_checking_memory(conv.src, conv.tgt))
    {
      ...
      Term tgt_mem = conv.inst_as_array(conv.tgt.memory);
      Term tgt_mem_indef = conv.inst_as_array(conv.tgt.memory_indef);
