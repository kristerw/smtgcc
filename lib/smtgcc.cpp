...
Inst *create_inst(Op op, Inst *arg1, Inst *arg2)
{
  ...
  else if (op == Op::LOAD_BE
	   || op == Op::LOAD_LE
	   || op == Op::GET_MEM_FLAG_BE
	   || op == Op::GET_MEM_FLAG_LE
	   || op == Op::GET_MEM_INDEF_BE
	   || op == Op::GET_MEM_INDEF_LE)
    {
      assert(arg1->bitsize == arg1->bb->func->module->ptr_bits);
      assert(arg2->op == Op::VALUE);
      inst->bitsize = arg2->value() * 8;
    }
  else if (op == Op::STORE_BE
	   || op == Op::STORE_LE
	   || op == Op::SET_MEM_FLAG_BE
	   || op == Op::SET_MEM_FLAG_LE
	   || op == Op::SET_MEM_INDEF_BE
	   || op == Op::SET_MEM_INDEF_LE)
    {
      assert(arg1->bitsize == arg1->bb->func->module->ptr_bits);
